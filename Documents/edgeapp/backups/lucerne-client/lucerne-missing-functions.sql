-- ================================================================
-- LUCERNE CLIENT - MISSING DATABASE FUNCTIONS RESTORATION
-- Deploy missing functions causing 404 errors in client application
-- Apply to: wvggehrxhnuvlxpaghft.supabase.co
-- ================================================================

-- Update tenant check to use 'lucerne' instead of 'edge'
-- This fixes role checking for the Lucerne client deployment
...
AS $$
    SELECT json_build_array(
        ...
        json_build_object(
            'question_id', gen_random_uuid(),
            'question_text', 'How clear are your responsibilities?',
            'avg_score', 4.0,
            'response_count', COALESCE(
                (SELECT COUNT(*) FROM team_health_pulse_responses r
                 JOIN employees e ON r.employee_id = e.id
...
-- ================================================================
-- FEEDBACK AND DEVELOPMENT FUNCTIONS  
-- ================================================================

-- Function: get_feedback_wall() - Feedback wall display
...
AS $$
    SELECT 
        ...
        f.created_at
    FROM feedback f
    JOIN employees giver ON f.giver_id = giver.id
    JOIN employees receiver ON f.receiver_id = receiver.id
    ...
    SELECT 
        k.id::bigint as feedback_id,
        COALESCE(giver.first_name || ' ' || giver.last_name, giver.name) as giver_name,
        COALESCE(receiver.first_name || ' ' || receiver.last_name, receiver.name) as receiver_name,
        k.comment as feedback_text,
        'kudos' as feedback_type,
        k.created_at
    FROM kudos k
    JOIN employees giver ON k.given_by = giver.id
    JOIN employees receiver ON k.employee_id = receiver.id
    WHERE giver.tenant_id = 'lucerne'
    AND receiver.tenant_id = 'lucerne'
...
-- Function: get_my_feedback_given() - User's given feedback
...
BEGIN
    ...
    FROM feedback f
    JOIN employees receiver ON f.receiver_id = receiver.id
    WHERE f.giver_id = current_employee_id
    ...
    FROM kudos k
    JOIN employees receiver ON k.employee_id = receiver.id
    WHERE k.given_by = current_employee_id
...
-- Multiple methods to force PostgREST to recognize new functions
NOTIFY pgrst, 'reload schema';
COMMENT ON SCHEMA public IS 'Updated with missing functions for Lucerne client';

SELECT 'All missing functions created successfully for Lucerne client' as status;